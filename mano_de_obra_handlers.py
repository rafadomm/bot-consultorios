...
# Añadimos un estado final para esperar la acción del usuario
...
async def start_mo_query(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not weeks:
        ...
        # Usamos el fallback para salir limpiamente
...
async def generate_mo_report(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Genera y muestra el reporte de desglose, y entra en estado de espera."""
    query = update.callback_query
    await query.answer()
    ...
    # En lugar de terminar, entramos en un estado de espera.
    return REPORT_DISPLAYED

# --- CONVERSATION HANDLER (SIMPLIFICADO Y ROBUSTO) ---
mano_de_obra_conv_handler = ConversationHandler(
    entry_points=[
        CallbackQueryHandler(restricted(start_mo_query), pattern="^start_mo$")
    ],
    states={
        ...
        # En este estado, el bot no hace nada, solo espera a los fallbacks.
        REPORT_DISPLAYED: [
             # Se necesita un handler aquí, aunque no haga nada, para mantener la conversación activa.
             # La salida real se maneja en los fallbacks.
        ] 
    },
    fallbacks=[
        # Este fallback ahora funcionará desde CUALQUIER estado, incluyendo REPORT_DISPLAYED.
