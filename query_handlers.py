...
async def mostrar_compras(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for fecha_str in sorted(compras_agrupadas.keys()):
        ...
        try: fecha_obj = datetime.strptime(fecha_str, '%Y-%m-%d'); fecha_formateada = fecha_obj.strftime('%d-%m-%Y')
        ...
        if fecha_obj and project_start_date:
            delta = fecha_obj - project_start_date; project_week = (delta.days // 7) + 1
        ...
        else: mensaje_actual_telegram += bloque_dia_telegram
    ...
    if weekly_totals:
        ...
        else: mensaje_actual_telegram += summary_header + summary_text_telegram; mensajes_telegram.append(header + mensaje_actual_telegram)
        mensaje_para_imprimir += summary_text_printer
    else: mensajes_telegram.append(header + mensaje_actual_telegram)
...
query_conv_handler = ConversationHandler(
    entry_points=[CallbackQueryHandler(restricted(solicitar_proveedor), pattern="^consultar_compras$")],
    states={
        SELECTING_PROVIDER: [
