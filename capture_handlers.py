...
# Definimos todos los estados necesarios para el flujo completo
...
# --- Flujo de Creación de Proveedor ---
...
async def receive_new_provider_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    return NEW_PRODUCT_NAME_FOR_NEW_PROVIDER
...
# --- Flujo de Creación de Producto para Proveedor Existente (RESTAURADO) ---
...
# --- Flujo de Cantidad, Precio, Resumen y Guardado (Estable) ---
...
async def ask_for_new_price(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    await query.edit_message_text("OK. Por favor, escribe el nuevo precio unitario:")
...
def _add_item_to_cart(context: ContextTypes.DEFAULT_TYPE, price: float):
    ...
    context.user_data['current_purchase']['items'].append({
        ...
        'quantity': product['quantity'], 'price': price
    })
...
# --- CONVERSATION HANDLER (Definitivo y Corregido) ---
capture_conv_handler = ConversationHandler(
    ...
    states={
        ...
        AWAITING_NEW_PRODUCT_NAME: [
             MessageHandler(filters.TEXT & ~filters.COMMAND, receive_new_product_name_for_existing_provider)
        ],
