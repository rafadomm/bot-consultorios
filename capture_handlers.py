...
import baserow_queries, baserow_commands, query_handlers
...
# Estados
(SELECT_PROVIDER, NEW_PROVIDER_NAME, NEW_PRODUCT_NAME_FOR_NEW_PROVIDER, SELECT_PRODUCT, 
 AWAITING_NEW_PRODUCT_NAME, ENTER_QUANTITY, AWAITING_PRICE_CHOICE, ENTERING_NEW_PRICE, 
 ADD_ANOTHER_OR_FINISH, AWAITING_RECEIPT_CHOICE, AWAITING_RECEIPT_UPLOAD) = range(11)
...
async def receive_new_provider_name(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(f"✅ ¡Proveedor creado!\n\nAhora, escribe el nombre del primer producto:"); return NEW_PRODUCT_NAME_FOR_NEW_PROVIDER

async def receive_new_product_name_for_new_provider(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not new_product: await update.message.reply_text("❌ Error. Escribe de nuevo el nombre."); return NEW_PRODUCT_NAME_FOR_NEW_PROVIDER
...
async def ask_product(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for p in products: keyboard.append([InlineKeyboardButton(p.get('PRODUCTO'), callback_data=f"id_prod_{p.get('id')}_{p.get('PRODUCTO')}")])
    keyboard.append([InlineKeyboardButton("❌ Cancelar", callback_data="cancel_capture")])
    await update.callback_query.edit_message_text("Selecciona un producto (o agrégalo si es nuevo):", reply_markup=InlineKeyboardMarkup(keyboard)); return SELECT_PRODUCT
...
async def ask_new_product_name_for_existing_provider(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    return AWAITING_NEW_PRODUCT_NAME

async def receive_new_product_name_for_existing_provider(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    product_name = update.message.text
    provider_id = context.user_data['current_purchase']['provider_id']; provider_name = context.user_data['current_purchase']['provider_name']
    ...
    if not new_product: await update.message.reply_text("❌ Hubo un error. Intenta de nuevo."); return AWAITING_NEW_PRODUCT_NAME
    context.user_data['current_product'] = {'id': new_product.get('id'), 'name': product_name}
    await update.message.reply_text(f"✅ ¡Producto '{product_name}' guardado!\n\nAhora, ¿qué cantidad compraste?"); return ENTER_QUANTITY
...
async def receive_quantity_and_ask_price(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(f"El último precio registrado es de ${last_price:,.2f}.\n¿Usamos este precio?", reply_markup=InlineKeyboardMarkup(keyboard)); return AWAITING_PRICE_CHOICE

async def ask_for_new_price(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query; await query.answer()
    await query.edit_message_text("OK. Por favor, escribe el nuevo precio unitario:"); return ENTERING_NEW_PRICE
...
async def ask_for_receipt(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query; await query.answer()
    keyboard = [[InlineKeyboardButton("📸 Adjuntar Foto o PDF", callback_data="attach_receipt")], [InlineKeyboardButton("⏩ Omitir y Guardar", callback_data="skip_receipt")]]
    await query.edit_message_text("¿Deseas adjuntar el comprobante de esta compra?", reply_markup=InlineKeyboardMarkup(keyboard)); return AWAITING_RECEIPT_CHOICE

async def ask_for_receipt_upload(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query; await query.answer()
    await query.edit_message_text("OK. Envía la foto o el archivo PDF del comprobante."); return AWAITING_RECEIPT_UPLOAD
...
async def save_purchase(update: Update, context: ContextTypes.DEFAULT_TYPE, receipt_data=None) -> int:
    ...
    if result: await message.reply_text("✅ ¡Compra guardada exitosamente!")
    else: await message.reply_text("❌ Hubo un error al guardar tu compra.")
    context.user_data.clear(); await asyncio.sleep(2); return await query_handlers.start(update, context)
...
async def receive_receipt_and_save(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not attachment: await update.message.reply_text("No enviaste un archivo."); return AWAITING_RECEIPT_UPLOAD
    await update.message.reply_text("Descargando y subiendo comprobante... ⏳"); file = await attachment.get_file(); file_bytes = await file.download_as_bytearray()
    file_name = attachment.file_name or "comprobante.jpg"; receipt_data = baserow_commands.upload_file(file_bytes, file_name)
    if not receipt_data: await update.message.reply_text("❌ Hubo un error al subir. Guardando sin comprobante...")
...
capture_conv_handler = ConversationHandler(
    entry_points=[CallbackQueryHandler(restricted(start_capture), pattern="^start_capture$")],
    states={
        ...
        NEW_PRODUCT_NAME_FOR_NEW_PROVIDER: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_new_product_name_for_new_provider)],
        SELECT_PRODUCT: [ CallbackQueryHandler(select_product, pattern="^id_prod_"), CallbackQueryHandler(ask_new_product_name_for_existing_provider, pattern="^new_product$")],
        AWAITING_NEW_PRODUCT_NAME: [ MessageHandler(filters.TEXT & ~filters.COMMAND, receive_new_product_name_for_existing_provider) ],
        ...
        ADD_ANOTHER_OR_FINISH: [ CallbackQueryHandler(ask_product, pattern="^add_another$"), CallbackQueryHandler(ask_for_receipt, pattern="^finish_items$")],
