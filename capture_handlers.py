...
(SELECT_PROVIDER, NEW_PROVIDER_NAME, NEW_PRODUCT_NAME_FOR_NEW_PROVIDER, SELECT_PRODUCT, 
 ENTER_QUANTITY, AWAITING_PRICE_CHOICE, ENTERING_NEW_PRICE, 
 ADD_ANOTHER_OR_FINISH, AWAITING_NEW_PRODUCT_NAME,
 AWAITING_RECEIPT_CHOICE, AWAITING_RECEIPT_UPLOAD) = range(6, 17)
...
async def receive_new_product_name_for_existing_provider(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    product_name = update.message.text
    provider_id, provider_name = context.user_data['current_purchase']['provider_id'], context.user_data['current_purchase']['provider_name']
    ...
    if not new_product:
        await update.message.reply_text("❌ Hubo un error. Intenta de nuevo."); return AWAITING_NEW_PRODUCT_NAME
    context.user_data['current_product'] = {'id': new_product.get('id'), 'name': product_name}
...
async def show_summary_and_ask_next(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    # --- ¡CORRECCIÓN! El botón de finalizar ahora apunta a la pregunta del comprobante ---
    keyboard = [[InlineKeyboardButton("[ + ] Añadir Otro Producto", callback_data="add_another")], [InlineKeyboardButton("✅ Finalizar Compra", callback_data="finish_items")]]
...
async def save_purchase(update: Update, context: ContextTypes.DEFAULT_TYPE, receipt_data=None) -> int:
    ...
    context.user_data.clear(); await asyncio.sleep(2)
...
async def receive_receipt_and_save(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not receipt_data: await update.message.reply_text("❌ Hubo un error al subir el archivo. Guardando sin comprobante...")
...
capture_conv_handler = ConversationHandler(
    ...
    states={
        ...
        NEW_PRODUCT_NAME_FOR_NEW_PROVIDER: [MessageHandler(filters.TEXT & ~filters.COMMAND, receive_new_product_name)],
        SELECT_PRODUCT: [
        ...
        ADD_ANOTHER_OR_FINISH: [ 
            ...
            # ¡CORRECCIÓN! El botón ahora llama a la función correcta
