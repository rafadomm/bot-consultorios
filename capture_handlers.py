...
# Añadimos nuevos estados para el flujo de precio
(SELECT_PROVIDER, NEW_PROVIDER_NAME, NEW_PRODUCT_NAME, SELECT_PRODUCT, 
 ENTER_QUANTITY, AWAITING_PRICE_CHOICE, ENTERING_NEW_PRICE, 
 ADD_ANOTHER_OR_FINISH) = range(6, 14)
...
async def show_summary_and_ask_next(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    resumen += f"\n*Total Acumulado: ${total_compra:,.2f}*"; keyboard = [[InlineKeyboardButton("[ + ] Añadir Otro Producto", callback_data="add_another")], [InlineKeyboardButton("✅ Finalizar y Guardar", callback_data="finish_capture")]]
...
async def finish_capture(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query; await query.answer(); await query.edit_message_text("Guardando tu compra en Baserow...")
    items_to_save = context.user_data['current_purchase']['items']; result = baserow_commands.create_batch_purchase(items_to_save)
...
capture_conv_handler = ConversationHandler(
    ...
    states={
        ...
        ADD_ANOTHER_OR_FINISH: [ CallbackQueryHandler(ask_product, pattern="^add_another$"), CallbackQueryHandler(finish_capture, pattern="^finish_capture$")]
    },
